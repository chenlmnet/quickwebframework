...
public abstract class QuickWebFrameworkFactory {
    ...
    protected void addSimpleServiceStaticFieldLink(String serviceName, String fieldName) {
        try {
            Field field = QuickWebFrameworkFactory.class.getDeclaredField(fieldName);
    ...
    public void startOSGiFreamwork(ServletContext servletContext) {
        String propertiesFileName = servletContext.getInitParameter(CONFIG_LOCATION_PARAMETER_NAME);
        ...
        String quickWebFrameworkPropertiesFilePath = servletContext.getRealPath(propertiesFileName);

        File quickWebFrameworkPropertiesFile = new File(quickWebFrameworkPropertiesFilePath);
        ...
        try {
            Reader quickWebFrameworkPropertiesReader =
                    new InputStreamReader(new FileInputStream(quickWebFrameworkPropertiesFilePath), "utf-8");
            InputStream quickWebFrameworkPropertiesInputStream =
                    new ReaderInputStream(quickWebFrameworkPropertiesReader);
            quickWebFrameworkProperties.load(quickWebFrameworkPropertiesInputStream);
        ...
        // 配置缓存保存路径
        String osgiFrameworkStorage = quickWebFrameworkProperties.getProperty("qwf.osgiFrameworkStorage");
        osgiFrameworkStorage = servletContext.getRealPath(osgiFrameworkStorage);
        if (osgiFrameworkStorage != null) {
            osgiFrameworkConfigMap.put("org.osgi.framework.storage", osgiFrameworkStorage);
        }

        // 读取固定配置
        String osgiFrameworkFactoryConfig = quickWebFrameworkProperties.getProperty("qwf.osgiFrameworkFactoryConfig");
        ...
        // 加载OSGi框架类或OSGi框架工厂类
        String osgiFrameworkClass = quickWebFrameworkProperties.getProperty("qwf.osgiFrameworkClass");
        if (osgiFrameworkClass != null) {
            Class<?> osgiFrameworkClazz;
            try {
                osgiFrameworkClazz = Class.forName(osgiFrameworkClass);
            } catch (ClassNotFoundException e) {
                throw new RuntimeException("查找osgiFrameworkClass类失败！-->" + osgiFrameworkClass, e);
            }
            ...
            try {
                frameworkConstructor = osgiFrameworkClazz.getConstructor(Map.class);
            } catch (SecurityException e) {
                throw new RuntimeException("获取osgiFrameworkClass的构造函数时出现安全异常。", e);
            } catch (NoSuchMethodException e) {
                throw new RuntimeException("未找到osgiFrameworkClass的Class(Map map)构造函数。", e);
            }
            try {
                framework = (Framework) frameworkConstructor.newInstance(osgiFrameworkConfigMap);
            } catch (Exception e) {
                throw new RuntimeException("初始化osgiFrameworkClass的实例时出现异常。", e);
            }
            logger.info("正在启动OSGi框架，OSGi框架类: " + osgiFrameworkClass);
        } else {
            String osgiFrameworkFactoryClass = quickWebFrameworkProperties.getProperty("qwf.osgiFrameworkFactoryClass");
            Class<?> osgiFrameworkFactoryClazz;
            try {
                osgiFrameworkFactoryClazz = Class.forName(osgiFrameworkFactoryClass);
            } catch (ClassNotFoundException e) {
            ...
            try {
                factory = (FrameworkFactory) osgiFrameworkFactoryClazz.newInstance();
            } catch (Exception e) {
        ...
        try {
            ...
            // 初始化插件管理Servlet
            tmpServlet = PluginManageServlet.initServlet(servletContext, quickWebFrameworkProperties);
            if (tmpServlet != null) {
                qwfServletList.add(tmpServlet);
            }

            // 将ServletContext注册为服务
            getBundleContext().registerService(ServletContext.class.getName(), servletContext, null);
            // 将Framework注册为服务
            getBundleContext().registerService(Framework.class.getName(), framework, null);

            // 将QuickWebFramework配置设置到ServletContext中
            servletContext.setAttribute(QWF_CONFIG_PROPERTY_KEY, quickWebFrameworkProperties);
            // 将当前Web app的类加载器设置到ServletContext中
            servletContext.setAttribute(QWF_WEBAPP_CLASSLOADER, this.getClass().getClassLoader());
            // 设置WEB根目录到系统配置中
            ...
            getBundleContext().addServiceListener(new ServiceListener() {
                ...
                public void serviceChanged(ServiceEvent event) {
                    String changedServiceName = event.getServiceReference().toString();
                    ...
                    // 日志特殊处理
                    if (changedServiceName.contains(CONST_SERVLET_FILTER_BRIDGE_CLASS_NAME)) {
                        // 刷新各日志的处理器
                        if (logBridgeObject != null) {
            ...
            this.addSimpleServiceStaticFieldLink(CONST_HTTP_SERVLET_BRIDGE_CLASS_NAME, "httpServletBridgeObject");
            this.addSimpleServiceStaticFieldLink(CONST_LOG_BRIDGE_CLASS_NAME, "logBridgeObject");
            this.addSimpleServiceStaticFieldLink(CONST_SERVLET_FILTER_BRIDGE_CLASS_NAME, "servletFilterBridgeObject");
            this.addSimpleServiceStaticFieldLink(CONST_SERVLET_LISTENER_BRIDGE_CLASS_NAME, "servletListenerBridgeObject");

            // 启动OSGi框架所用时间
            long startFrameworkUsedTime = System.currentTimeMillis() - osgiFrameworkStartTime;
            logger.info(String.format("启动OSGi框架完成，用时 %,d ms", startFrameworkUsedTime));
